...
public class Staff {
    ...
    private ArrayList<MeasureChordSymbol> chords;
    ...
    public void AddMeasureChords(ArrayList<MeasureChordSymbol> trackChords) {
        if (trackChords == null || trackChords.size() == 0) {
            return;
        }
        chords = new ArrayList<MeasureChordSymbol>();
        int xpos = 0;
        int symbolindex = 0;
        for (MeasureChordSymbol chord : trackChords) {
            if (chord.getStartTime() < starttime) {
                continue;
            }
            if (chord.getStartTime() > endtime) {
                break;
            }
            /* Get the x-position of this lyric */
            while (symbolindex < symbols.size() &&
                    symbols.get(symbolindex).getStartTime() < chord.getStartTime()) {
                xpos += symbols.get(symbolindex).getWidth();
                symbolindex++;
            }
            chord.setX(xpos);
            if (symbolindex < symbols.size() &&
                    (symbols.get(symbolindex) instanceof BarSymbol)) {
                chord.setX(chord.getX() + SheetMusic.NoteWidth);
            }
            chords.add(chord);
        }
        if (chords.size() == 0) {
    ...
    private void DrawChords(Canvas canvas, Paint paint) {
        /* Skip the left side Clef symbol and key signature */
        int xpos = keysigWidth;
        //int ypos = height - SheetMusic.NoteHeight * 3/2;

        int ypos  = ytop - SheetMusic.NoteHeight * 2;;
        for (MeasureChordSymbol chord : chords) {
            canvas.drawText(chord.getText(),
                    xpos + chord.getX(),
                    ypos,
                    paint);
        }
    ...
    public void Draw(Canvas canvas, Rect clip, Paint paint) {
        ...
        if (showMeasures) {
            ...
            number_paint.setTextSize(8.0f);
            Typeface typeface = Typeface.create(number_paint.getTypeface(), Typeface.NORMAL);
            number_paint.setTypeface(typeface);
            number_paint.setColor(Color.BLACK);
            DrawMeasureNumbers(canvas, number_paint);
        }
        if (lyrics != null) {
            DrawLyrics(canvas, paint);
        }
        if(chords != null){
            Paint chord_paint = new Paint();
            chord_paint.setTextSize(10.0f);
            Typeface chord_typeface = Typeface.create(chord_paint.getTypeface(), Typeface.NORMAL);
            chord_paint.setTypeface(chord_typeface);
            chord_paint.setColor(Color.BLACK);
            DrawChords(canvas,chord_paint);
        }
    }
        ...
        for (int i = 0; i < symbols.size(); i++) {
            ...
            if (redrawLines) {
                ...
                if(chords != null){
                    Paint chord_paint = new Paint();
                    chord_paint.setTextSize(10.0f);
                    Typeface typeface = Typeface.create(chord_paint.getTypeface(), Typeface.NORMAL);
                    chord_paint.setTypeface(typeface);
                    chord_paint.setColor(Color.BLACK);
                    DrawChords(canvas,chord_paint);
                }
            }
