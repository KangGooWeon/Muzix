...
package com.example.muzix.util;

import com.example.muzix.MidiFile;
import com.example.muzix.MidiTrack;
import com.example.muzix.TimeSignature;
...
public class MidiProcessor
{
    ...
    private HashMap<Class<? extends MidiEventWriter>, List<MidiEventListener>> mEventsToListeners;
    private HashMap<MidiEventListener, List<Class<? extends MidiEventWriter>>> mListenersToEvents;
    ...
    public MidiProcessor(MidiFile input)
    {
        ...
        mEventsToListeners = new HashMap<Class<? extends MidiEventWriter>, List<MidiEventListener>>();
        mListenersToEvents = new HashMap<MidiEventListener, List<Class<? extends MidiEventWriter>>>();
    ...
    public void registerEventListener(MidiEventListener mel, Class<? extends MidiEventWriter> event)
    {
        ...
        List<Class<? extends MidiEventWriter>> events = mListenersToEvents.get(mel);
        if(events == null)
        {

            events = new ArrayList<Class<? extends MidiEventWriter>>();
            events.add(event);
    ...
    public void unregisterEventListener(MidiEventListener mel)
    {
        ...
        for(Class<? extends MidiEventWriter> event : events)
        {
    ...
    public void unregisterEventListener(MidiEventListener mel, Class<? extends MidiEventWriter> event)
    {
    ...
    protected void dispatch(MidiEventWriter event)
    {
        ...
        this.sendOnEventForClass(event, MidiEventWriter.class);
    }

    private void sendOnEventForClass(MidiEventWriter event, Class<? extends MidiEventWriter> eventClass)
    {
    ...
    private void process()
    {
        ...
        while(mRunning)
        {
            ...
            for(int i = 0; i < mEventQueues.length; i++)
            {
                ...
                ArrayList<MidiEventWriter> events = queue.getNextEventsUpToTick(mTicksElapsed);
                for(MidiEventWriter event : events)
                {
                    this.dispatch(event);
                }
    ...
    private class MidiTrackEventQueue
    {
        ...
        private Iterator<MidiEventWriter> mIterator;
        private ArrayList<MidiEventWriter> mEventsToDispatch;
        private MidiEventWriter mNext;
        ...
        public MidiTrackEventQueue(MidiTrack track)
        {
            ...
            mEventsToDispatch = new ArrayList<MidiEventWriter>();
        ...
        public ArrayList<MidiEventWriter> getNextEventsUpToTick(double tick)
        {
