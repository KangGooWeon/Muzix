...
import com.example.muzix.ChannelEvent;
import com.example.muzix.SystemExclusiveEvent;
...
public abstract class MidiEventWriter implements Comparable<MidiEventWriter>, Serializable
{
    protected long mTick;
    protected VariableLengthInt mDelta;

    public MidiEventWriter(long tick, long delta)
    {
        mTick = tick;
    ...
    public boolean requiresStatusByte(MidiEventWriter prevEvent)
    {
        if(prevEvent == null)
        {
            return true;
        }
        if(this instanceof MidiEventWriter)
        {
            return true;
        }
    ...
    public static final MidiEventWriter parseEvent(long tick, long delta, InputStream in) throws IOException
    {
        in.mark(1);
        boolean reset = false;
